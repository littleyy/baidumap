...
func main() {
	ColorPrintln("****************************", 2)
	ColorPrintln("Power by yst studio.", 6)
	ColorPrintln("****************************", 2)
	fmt.Println()

	var mins, maxs, ok, fls string
	...
	GetAllFloor(minLng, maxLng, minLat, maxLat, startZ, endZ, nowTime)
...
// 获得所有层级瓦片图
func GetAllFloor(minLng float64, maxLng float64, minLat float64, maxLat float64, startZ int, endZ int, nowTime string) {
	// 创建存储瓦片图总文件夹
	os.MkdirAll("./tiles", os.ModePerm)
	...
	for i := maxX; i >= minX; i-- {
		func(i int, minY int, maxY int) {
			...
			for j := maxY; j >= minY; j-- {
				...
				out, _ := os.Create(dir + "/" + strconv.Itoa(j) + ".jpg")
